...
const AnswerOption: React.FC<{ label: string; value: number; onSelect: (value: number) => void; selectedValue: number | null; theme: Theme }> = ({ label, value, onSelect, selectedValue, theme }) => {
    ...
    const baseClasses = "text-center py-2 px-4 rounded-full cursor-pointer transition-all duration-200 transform font-semibold w-full";
    ...
    const selectedClasses = `${theme.accentBg} ${selectedTextColor} scale-105 shadow-lg`;
...
export const QuizScreen: React.FC<QuizScreenProps> = ({ questions, onComplete, theme }) => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-4 sm:p-6 md:p-8 bg-zinc-900/50 rounded-2xl shadow-2xl border border-zinc-700/50 backdrop-blur-sm">
      ...
      <div className="text-center">
        ...
        <h2 className="text-xl sm:text-2xl text-white font-medium mb-10 min-h-[8rem] sm:min-h-[6rem] flex items-center justify-center p-2">
